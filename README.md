...
```sh
cd ./build/ && docker-compose run chipplayerjs-build bash -c "npm run build-catalog && cp ./server/catalog* ./server/directories.json /usr/share/www/" && cd ../server/ && docker-compose restart
```
